...
class IpaTools {
    ...
    public function __construct($ipa, $view = false) {
        ...
        if (empty($view)) {
            ...
            $this->applink = $this->applink . $this->baseurl . $this->basedir . '/files/' . basename($this->folder) . '/manifest.plist';
        }
    }
            ...
            if ($zip) {
                while ($zip_entry = zip_read($zip)) {
                    ...
                    if ($fileinfo['basename'] == 'Info.plist' || $fileinfo['basename'] == $this->itunesartwork 
                        || $fileinfo['basename'] == 'embedded.mobileprovision') {
    ...
    private function createManifest($ipa) {
        if (file_exists(dirname(__FILE__) . '/cfpropertylist/CFPropertyList.php')) {
            ...
            $this->icon = ($plistArray['CFBundleIconFile'] != '' ? $plistArray['CFBundleIconFile'] 
                : (count($plistArray['CFBundleIconFile']) > 0 ? $plistArray['CFBundleIconFile'][0] : null));
            ...
            die('CFPropertyList class was not found! You need it to create the wireless manifest. Put it in de folder cfpropertylist!');
    }
